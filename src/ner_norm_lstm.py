...
class InputExample(object):
  ...
  def __init__(self, guid, text_a, text_b=None, start_labels=None, end_labels=None, \
                            norm_tag=None, postag_p=None, postag_q=None, bioes=None):
...
class NerProcessor(DataProcessor):
  ...
  def get_train_examples(self, data_dir):
    ...
    return self._create_examples(
        self._read_tsv(os.path.join(data_dir, "merge_dev2_raw.out")), "train")

  def get_dev_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
        self._read_tsv(os.path.join(data_dir, "test.out")), "dev")
    ...
    for (i, line) in enumerate(lines):
      ...
      examples.append(
          InputExample(guid=guid, text_a=text_a, text_b=text_b, start_labels=start_labels,\
                      end_labels=end_labels, norm_tag=norm_tag, postag_p=postag_p, postag_q=postag_q, bioes=bioes))
    return examples
...
def convert_single_example(ex_index, example, label_map, max_seq_length,
  ...
  ## 4 layer
  code_map_t = joblib.load(FLAGS.data_dir+'/code_id_t.pkl')
  code_map_b = joblib.load(FLAGS.data_dir+'/code_id_b.pkl')
  code_map_d = joblib.load(FLAGS.data_dir+'/code_id_d.pkl')
  code_map_h = joblib.load(FLAGS.data_dir+'/code_id_h.pkl')
  ...
  for i, word in enumerate(text_a):
    ...
    for m in range(len(token)):
      if m == 0:
        ...
        if tmp_bioes == 'B' or tmp_bioes == 'I' or tmp_bioes == 'O':
          all_bioes.append(tmp_bioes)
        elif tmp_bioes == 'S':
          all_bioes.append('B')
        elif tmp_bioes == 'E':
          all_bioes.append('I')
        else:
          assert tmp_bioes == 'B' or tmp_bioes == 'I' or tmp_bioes == 'E' or tmp_bioes == 'O' or tmp_bioes == 'S'
      else: # 一般不会出现else
        all_start_labels.append(0)
        all_end_labels.append(0)
        all_postag.append(tmp_postag)
        ...
        if tmp_bioes == 'B':
          all_bioes.append('I')
        elif tmp_bioes == 'I' or tmp_bioes == 'O':
          all_bioes.append(tmp_bioes)
        elif tmp_bioes == 'S' or tmp_bioes == 'E':
          if m == len(token)-1:
            all_bioes.append('E')
          else:
            all_bioes.append('I')
...
# process query sequence
  ...
  if len(tokens) >= max_seq_length - 1:
    ...
    all_bioes = all_bioes[:(max_seq_length - 1)]
...
## get wordshape features
  ...
  for norm in all_norm_tag:
    #把标签进行拆分成4层
    # print('kkkkkkk:',norm)
    if FLAGS.tag_layer == 4:
      if norm=='O':
        t = 'O'
        b = 'O'
        d = 'O'
        ...
        t, b, d, h = norm.aplit('/')
      if t not in code_map_t:
        all_norm_t_ids.append(0)
      else:
        all_norm_t_ids.append(code_map_t[t])
      if b not in code_map_b:
        all_norm_b_ids.append(0)
      else:
        all_norm_b_ids.append(code_map_b[b])
      if d not in code_map_d:
        all_norm_d_ids.append(0)
      else:
        all_norm_d_ids.append(code_map_d[d])
      if h not in code_map_h:
        all_norm_h_ids.append(code_map_h[h])
      else:
        all_norm_h_ids.append(code_map_h[h])
    elif FLAGS.tag_layer == 3:
      if norm == 'O':
        t = 'O'
        bd = 'O'
        h = 'O'
  ...
  all_bioes_ids = []
  for bioes in all_bioes:
    if bioes == 'O':
      all_bioes_ids.append(0)
    elif bioes == 'B':
      all_bioes_ids.append(1)
    elif bioes == 'I':
      all_bioes_ids.append(2)
    elif bioes == 'E':
      all_bioes_ids.append(3)
    elif bioes == 'S':
      all_bioes_ids.append(4)
    else:
      all_bioes_ids.append(0)
  ...
  assert len(all_norm_t_ids) == max_seq_length
  assert len(all_norm_b_ids) == max_seq_length
  assert len(all_norm_d_ids) == max_seq_length
  assert len(all_norm_h_ids) == max_seq_length
  ...
  assert len(all_bioes_ids) == max_seq_length
  ...
  if ex_index < 5:
    ...
    tf.logging.info("all_bioes_ids: %s" % " ".join([str(x) for x in all_bioes_ids]))
...
def file_based_convert_examples_to_features(
  ...
  for (ex_index, example) in enumerate(examples):
    ...
    features["bioes_ids"] = create_int_feature(feature.bioes_ids)
...
def file_based_input_fn_builder(input_file, seq_length, is_training,
  ...
  name_to_features = {
      ...
      "norm_t_ids": tf.FixedLenFeature([seq_length], tf.int64),
      "norm_b_ids": tf.FixedLenFeature([seq_length], tf.int64),
      "norm_d_ids": tf.FixedLenFeature([seq_length], tf.int64),
      "norm_h_ids": tf.FixedLenFeature([seq_length], tf.int64),
      "postag_ids": tf.FixedLenFeature([seq_length], tf.int64),
      "bioes_ids": tf.FixedLenFeature([seq_length], tf.int64),
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,\
                 start_labels_ids, end_labels_ids, norm_t_ids, norm_b_ids, norm_d_ids, norm_h_ids,\
                  postag_ids, wordshape_ids, chars_ids, num_labels, id2code_t, \
  ...
  with tf.variable_scope('pos_embedding'):
    pos_embedding = tf.get_variablele("pos_embedding", shape=[209, FLAGS.pos_embedding_dim], \
                  dtype=tf.float32, initializer=initializers.xavier_initializer())
    embed_pos = tf.nn.embedding_lookup(pos_embedding, postag_ids)
    pos_rep = tf.nn.tanh(embed_pos)
  ## BIOES feature
  with tf.variable_scope('bioes_embedding'):
    bioes_embedding = tf.get_variablele("bioes_embedding", shape=[5, FLAGS.bioes_embedding_dim], \
                  dtype=tf.float32, initializer=initializers.xavier_initializer())
    embed_bioes = tf.nn.embedding_lookup(bioes_embedding, bioes_ids)
    bioes_rep = tf.nn.tanh(embed_bioes)
 ...
 ##### BERT + SAC
  ...
  norm_feature = tf.concat([output_layer, bioes_rep], axis=-1)
  with tf.variable_scope('biLSTM_norm'):
    lstm_output_norm = tf.keras.layers.LSTM(FLAGS.lstm_size, return_sequences=True)(norm_feature)
  norm_output = tf.reshape(lstm_output_norm, shape=[-1, FLAGS.lstm_size])
  
  if is_training:
    output_layer = tf.nn.dropout(output_layer, keep_prob=0.9)
    norm_output = tf.nn.dropout(norm_output, keep_prob=0.9)
  ##### BERT + Feature + SAC
  ...
  norm_hidden = tf.reshape(norm_output, shape=[-1, flags.lstm_size])
  ...
  with tf.variable_scope("norm_logits"):
    for i in range(len(id2code)):
      ...
      norm_b = tf.get_variable("norm_b_"+str(i), shape=[len(id2code[i])], dtype=tf.float32,
                          initializer=tf.zeros_initializer())
      norm_pred.append(tf.nn.xw_plus_b(norm_hidden, norm_W, norm_b))
  ...
  with tf.variable_scope("norm_loss"):
    ...
    norm_loss = []
    for i in range(len(id2code)):
      logits_norm = tf.reshape(norm_pred[i], [-1, FLAGS.max_seq_length, len(id2code[i])])
  ...
  ## BERT + SAC
  if FLAGS.tag_layer == 3:
    all_norm_loss = norm_loss[0] + norm_loss[1] + norm_loss[2] + norm_loss[3]
  else:
    all_norm_loss = norm_loss[0] + norm_loss[1] + norm_loss[3]
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate, other_learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if FLAGS.tag_layer == 4:
      id2code_t = joblib.load(FLAGS.data_dir+'/id_code_t.pkl')
      id2code_b = joblib.load(FLAGS.data_dir+'/id_code_b.pkl')
      id2code_d = joblib.load(FLAGS.data_dir+'/id_code_d.pkl')
      id2code_h = joblib.load(FLAGS.data_dir+'/id_code_h.pkl')
    elif FLAGS.tag_layer == 3:
    ...
    (total_loss, logits, start_pred_ids, end_pred_ids, norm_pred) = create_model(
        bert_config, is_training, input_ids, input_mask, segment_ids, start_labels_ids,\
        end_labels_ids, norm_t_ids,norm_b_ids,norm_d_ids,norm_h_ids, postag_ids, wordshape_ids, chars_ids, num_labels,\
        id2code_t, id2code_b, id2code_d, id2code_h, cnn_extractor, tag_attention, use_one_hot_embeddings)
    pred_ids = tf.stack([start_pred_ids, end_pred_ids, norm_pred[0],norm_pred[1],norm_pred[2],norm_pred[3]],axis=1)
...
def get_pred_metric(predictions, eval_input_ids, tokenizer):
  ...
  all_pred_ent = []
  all_pred_norm = []
  all_correspond_norm = [] #entity对应的标准化信息
  for i, predict in enumerate(predictions):
    # print(i)
    # print(code_pred)
    tmp_input_ids = eval_input_ids[i]
    start_preds = list(predict[0])
    end_preds = list(predict[1])
    norm_pred_t = list(predict[2])
    norm_pred_b = list(predict[3])
    norm_pred_d = list(predict[4])
    norm_pred_h = list(predict[5])
    ...
    if FLAGS.tag_layer == 4:
      for j, idx in enumerate(norm_pred_t):
        norm_t = id2code_t[idx]
        norm_b = id2code_b[norm_pred_b[j]]
        norm_d = id2code_d[norm_pred_d[j]]
        norm_h = id2code_h[norm_pred_h[j]]
        n = ''
        if norm_t == 'O':
          continue
        if norm_b == 'O':
    ...
    if(len(start_inds)==0):
      ...
      if(len(ans)==0):
        ...
        for i, item in enumerate(ans):
          ...
          correspond_norm.append(norm[s_ind])
          
          # print(all_tmp_ent)
        all_pred_ent.append(all_tmp_ent)
        all_correspond_norm.append(correspond_norm)
  ...
  ## save result in file
  with open(os.path.join(FLAGS.output_dir, 'test.txt'), 'w') as f:
  ...
  with open(os.path.join(FLAGS.output_dir, 'test_pred_norm.txt'), 'w') as f:
  ...
  with open(os.path.join(FLAGS.output_dir, 'test_pred_task2.txt'), 'w') as f:
    for k, norm in enumerate(all_correspond_norm):
      if len(all_pred_ent[k]) == 0:
        f.write('\n')
      else:
        f.write('\t'.join(norm) + '\n')
