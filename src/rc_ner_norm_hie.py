...
class NerProcessor(DataProcessor):
  ...
  def get_dev_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
        self._read_tsv(os.path.join(data_dir, "dev2_raw.out")), "dev")

  def get_test_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
        self._read_tsv(os.path.join(data_dir, "dev2_raw.out")), "test")
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,\
 ...
 ##### BERT + SAC
  ...
  with tf.variable_scope("norm_logits"):
    for i in range(len(id2code)):
      ...
      norm_b = tf.get_variable("norm_b_"+str(i), shape=[len(id2code[i])], dtype=tf.float32,
                          initializer=tf.zeros_initializer())
      norm_pred.append(tf.nn.xw_plus_b(hidden, norm_W, norm_b))
  ...
  total_loss = start_loss + end_loss + all_norm_loss
...
def get_pred_metric(predictions, eval_input_ids, tokenizer):
  ...
  ## save result in file
  with open(os.path.join(FLAGS.output_dir, 'dev2_pred_answer.txt'), 'w') as f:
  ...
  with open(os.path.join(FLAGS.output_dir, 'dev2_pred_norm.txt'), 'w') as f:
